...
public class LabIPDPanel extends WebPanel implements Observer{
    ...
    public void getReq() {
		...
		tree1.setModel(getDefaultTreeModel(oUserInfo.GetPtAN())); 
	    ...
	    tree1.addTreeSelectionListener(new TreeSelectionListener(){
            ...
            public void valueChanged(TreeSelectionEvent e ) {
	    		...
	    		//getDataLabClear();
	    		String req=node.substring(node.indexOf("Req:")+4,node.indexOf("]"));
	    		//System.out.println(req+"<<<");
	    		getDataLab(req,(yr+"-"+month+"-"+ary[0]));
	...
	public WebTreeModel getDefaultTreeModel (String an){
    	//System.out.println(">>>"+hn);
    	UniqueNode root = new UniqueNode ( "AN :"+an );
		UniqueNode parent=null;
		String query =  "select  distinct entrydatetime from labreq where requestfroman is not null and cxlbyusercode is  null and labreq.requestfroman=? order by entrydatetime desc ";
		String query_ =  "select  requestno,bedno from labreq where requestfroman=? and entrydatetime=?";
		...
		try {
	        ...
	        while (rs.next()) {
	        	...
	        	//String req=rs.getString(2).trim();
	        	//String bedno=rs.getString(3).trim();
	        	//String reqno=rs.getString(4).trim();
	        	//System.out.println(">>>"+vn+"--"+lab_date+"----"+clinic);
	        	
	        	//parent = new UniqueNode (Setup.ShowThaiDateShort1(lab_date)+" [Req:"+req+"]"+"[Bed:"+bedno+"]") ;
	        	parent = new UniqueNode (Setup.ShowThaiDateShort1(lab_date)) ;
	        	
	        	stmt1 = conn.prepareStatement(query_);
				stmt1.setString(1,an);
				stmt1.setString(2,rs.getString(1).trim());
		        ResultSet rs1 = stmt1.executeQuery();
		        while (rs1.next()) {
		        	String req=rs1.getString(1).trim();
		        	String bedno=rs1.getString(2).trim();
		        	parent.add(new UniqueNode (" [Req:"+req+"]"+"[Bed:"+bedno+"]"));
		        }
	        	stmt1.close();
	...
	public  DefaultTableModel fetchDataLab(String req,String visitdate){
		Vector<Vector<Object>> dataLab = new Vector<Vector<Object>>();
			...
			String query1="select labresult.labcode,sysconfig.englishname, labresult.resultvalue,labresult.resultunitcode,labresult.normalresultvalue from labresult,sysconfig where labresult.labcode=sysconfig.code and labresult.hn='"+oUserInfo.GetPtHN()+"' and labresult.requestno='"+req_no+"'   and sysconfig.ctrlcode='20067'  and labresult.cxldatetime is null order by labresult.labcode "; 
